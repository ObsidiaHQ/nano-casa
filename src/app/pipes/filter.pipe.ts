...
export class FilterPipe implements PipeTransform {
  transform(
    ...
    if (query?.length < 3 && type === 'repo') return values as IRepo[];
    if (query?.length < 3 && type === 'user') return values as IContributor[];

    if (type === 'repo') {
      return [...values].filter((value: IRepo) => {
        return (
      ...
      return [...values].filter((value: IContributor) => {
        return (
